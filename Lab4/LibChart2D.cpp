...
CSolarSystem::CSolarSystem()
{
	double rS = 100,//радиус солнца
		rE = 70,//радиус земли	
		rM = 30,//радиус луны
		rMerc = 60;//радиус меркурия
	double rMy = 45; //Радиус моей планеты

	double RoE = 10 * rS//радиус от солнца до земли
		, RoM = 2 * rE,//радиус от земли до луны
		RoMerc = 5 * rS,//радиус от солнца до марса
		RoV = 10 * rS;
...
void CSolarSystem::setCoords()
{
	...
	_moonCoords = P * _moonCoords;
	_myPlanetCoords = P * _myPlanetCoords; // Для вражения вокруг другой планеты
...
void CSolarSystem::draw(CDC& dc)
{
	...
	d = _myPlanetOrbit.right; // Отрисовка орбиты моей планеты вокруг земли
	//R.SetRect(_earthCoords(0) - d, _earthCoords(1) + d, _earthCoords(0) + d, _earthCoords(1) - d);
	dc.Ellipse(R);
